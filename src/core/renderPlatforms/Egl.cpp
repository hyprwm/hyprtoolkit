...
CEGL::CEGL(wl_display* display) {
    ...
    if (eglDisplay == EGL_NO_DISPLAY) {
        g_logger->log(HT_LOG_CRITICAL, "Failed to create EGL display");
        goto error;
    }

    if (eglInitialize(eglDisplay, nullptr, nullptr) == EGL_FALSE) {
        g_logger->log(HT_LOG_CRITICAL, "Failed to initialize EGL");
        goto error;
    }

    if (!eglChooseConfig(eglDisplay, config_attribs, &eglConfig, 1, &matched)) {
        g_logger->log(HT_LOG_CRITICAL, "eglChooseConfig failed");
        goto error;
    }
    if (matched == 0) {
        g_logger->log(HT_LOG_CRITICAL, "Failed to match an EGL config");
        goto error;
    }

    eglContext = eglCreateContext(eglDisplay, eglConfig, EGL_NO_CONTEXT, context_attribs);
    if (eglContext == EGL_NO_CONTEXT) {
        g_logger->log(HT_LOG_CRITICAL, "Failed to create EGL context");
        goto error;
    }
